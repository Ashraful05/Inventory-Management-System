...
<head>
    ...
    <title>Customr Wise Paid Report</title>
</head>
<body>
<div class="container">
    <div class="row">
            ...
            <table border="1" width="100%">
                ...
                @foreach($paids as $key => $paid)
                    <tr>
                        ...
                        <td>
                            {{$paid['customer']['name']}}
                            ({{$paid['customer']['mobile_no']}} - {{$paid['customer']['address']}})
                        </td>
                        <td class="center">Invoice No# {{$paid['invoice']['invoice_no']}}</td>
                        <td class="center">{{ date('d-m-Y',strtotime($paid['invoice']['date'] )) }}</td>
                        <td class="center">Tk. {{$paid->paid_amount}}</td>
                        @php
